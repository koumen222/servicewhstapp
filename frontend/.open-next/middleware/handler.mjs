...
Learn More: https://nextjs.org/docs/messages/node-module-in-edge-runtime`;
  ...
  `);
      ...
      let bs = ["/", "/login", "/register", "/pricing"], bt = ["/admin"];
      function bu(a10) {
        let { pathname: b2 } = a10.nextUrl, c2 = a10.cookies.get("auth_token")?.value, d2 = ["/login", "/register"].some((a11) => b2.startsWith(a11)), e2 = bs.some((a11) => "/" === a11 ? "/" === b2 : b2.startsWith(a11));
        if (bt.some((a11) => b2.startsWith(a11)), "/" === b2) return S.next();
        if (!e2 && !c2) {
      ...
      let bv = { matcher: ["/((?!api|_next/static|_next/image|favicon.ico|.*\\.png|.*\\.svg).*)"] };
